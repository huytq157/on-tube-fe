...
const VideoCard = ({ video }: { video: Video }) => {
  ...
  const handleLoadedMetadata = (event: React.SyntheticEvent<HTMLVideoElement>) => {
    ...
    setDuration(`${minutes}:${remainingSeconds < 10 ? '0' : ''}${remainingSeconds}`)
  }
  ...
  return (
    <div>
      ...
      >
        <Link href={`/video/${video?._id}`} aria-label={`Xem video: ${video?.title}`}>
          {video?.videoThumbnail && (
            <Image
              src={video?.videoThumbnail || '.././../../public/no-image.png'}
              width={315}
              height={180}
              alt={video?.title}
              priority={false}
      ...
      <div className='flex gap-[2px] md:mt-1 '>
        ...
        <div className='flex-1 pr-[20px]'>
          ...
          <div className='flex gap-[2px] text-[14px] text-[#606060] font-medium'>
            <span>{video?.totalView} lượt xem</span>
            <span>•</span>
            <span>{calculateCreatedTime(video?.createdAt)}</span>
          </div>
        </div>
      ...
      <ModalSave open={isModalOpen} setIsModalOpen={setIsModalOpen} videoId={video?._id} />
    </div>
  )
}
