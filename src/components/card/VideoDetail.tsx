...
import { useDescViewAuthMutation, useDescViewMutation } from '@/redux/api/videoApi'
import { useUser } from '@/hook/AuthContext'
import { VideoCard } from '../types'
import Head from 'next/head'
import { Spin } from 'antd'
const VideoInfoWriter = dynamic(() => import('../shared/VideoInfoWriter'), { ssr: true })
  ...
  return (
    <div className='mb-10'>
      <Head>
        ...
        <meta property='og:url' content={`https://yourwebsite.com/video/${id}`} />
        <meta property='og:image' content={video?.videoThumbnail || '/default-thumbnail.jpg'} />
      </Head>
      <div className='w-full bg-black rounded-[10px]'>
        {video?.videoUrl ? (
          ...
          <div className='w-full bg-slate-200 md:h-[550px] sm:h-[220px] rounded-[10px] overflow-hidden'>
            <span className='text-center text-gray-600'>Không có video để hiển thị</span>
          </div>
        )}
      </div>
      ...
      <div className='bg-[#f2f2f2] rounded-[5px] mt-[20px] mb-[24px] p-[10px]'>
        <div className='flex gap-[5px] flex-wrap mb-2 text-[#606060] font-semibold'>
          <span className='text-[14px] font-roboto '>{video?.totalView} lượt xem</span>
          <span className='text-[14px]'>•</span>
          <span className='text-[14px] font-roboto'>{calculateCreatedTime(video?.createdAt)}</span>
        ...
        <div className='font-roboto'>
          {renderHTML(isExpanded ? video?.description : video?.description.slice(0, 300) + '...')}
          <button
            type='button'
      ...
      {video?.allowComments === true ? (
        ...
        <p className='font-[500] font-roboto text-center'>Nhà sáng tạo đã tắt bình luận</p>
      )}
    </div>
  )
}
